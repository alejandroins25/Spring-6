...
public class TrackingImpl implements ConstraintValidator<Tracking, List<Action>> {
	...
	private static final Type LAST_ACTION = Type.DELIVERY;

	private static final Map<Type, List<Type>> ACTION_AND_ITS_ALLOWED_PREVIOUS_ACTIOS = new EnumMap<>(Type.class);

	static {
		ACTION_AND_ITS_ALLOWED_PREVIOUS_ACTIOS.put(Type.RECEPTION, new ArrayList<>());
		ACTION_AND_ITS_ALLOWED_PREVIOUS_ACTIOS.put(Type.ASSIGNMENT, Arrays.asList(Type.RECEPTION));
		ACTION_AND_ITS_ALLOWED_PREVIOUS_ACTIOS.put(Type.DELIVERY, Arrays.asList(Type.ASSIGNMENT));
	}
	...
	private boolean isAlreadyClosed(ConstraintValidatorContext context, Action action, int pos) {
		...
		if (alreadyClosed) {
			...
			closed = LAST_ACTION.equals(action.getType());
		}
